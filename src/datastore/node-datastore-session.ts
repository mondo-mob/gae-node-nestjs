...
import { DeleteCallback, SaveCallback } from '@google-cloud/datastore/build/src/request';
...
export class DatastoreStore extends expressSession.Store {
  ...
  constructor(options: DataStoreOptions = {}) {
    ...
    if (options.expirationMs) {
      if (typeof options.expirationMs !== typeof 0 || options.expirationMs < 0) {
  ...
  get = (sid: string, callback: (err: Error | null, session?: expressSession.SessionData | null) => void) => {
  ...
  set = (sid: string, sess: expressSession.SessionData, callback?: (err: Error | null) => void) => {
    callback = callback || (() => {});
    let sessJson;
